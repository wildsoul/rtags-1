  ...
  (save-excursion
    (let ((rc (rtags-executable-find "rc")) proc)
      ...
      (cond ((stringp path) (push (concat "--with-project=" path) arguments))
            ...
            (t nil))
      (if context
          (push (concat "-t=" context) arguments))
...
(defun rtags-index-js-file ()
  (interactive)
  (if (buffer-file-name)
      (rtags-call-rc "--compile" (buffer-file-name))))
...
(defun rtags-async-rc-sort-lines-compare (l r)
  (let ((strl l)
        (strr r)
        (lpath "")
        (rpath "")
        ...
        (ldef nil)
        (rdef nil))
    (if (string-match "^\\(.*\\):\\([0-9]+\\):\\([0-9]+\\) \\(.\\)" strl)
        (setq lpath (match-string 1 strl)
              lline (string-to-int (match-string 2 strl))
              lcol (string-to-int (match-string 3 strl))
              ldef (rtags-is-definition (match-string 4 strl))))
    (if (string-match "^\\(.*\\):\\([0-9]+\\):\\([0-9]+\\) \\(.\\)" strr)
        (setq rpath (match-string 1 strr)
              rline (string-to-int (match-string 2 strr))
              rcol (string-to-int (match-string 3 strr))
              rdef (rtags-is-definition (match-string 4 strr))))
    (cond ((not (eq ldef rdef)) ldef)
          ((not (string= lpath rpath)) (string< lpath rpath))
          ((not (= lline rline)) (< lline rline))
          (t (< lcol rcol))))
  )

(defun rtags-async-rc-sort-lines ()
  (let ((lines (sort (split-string (buffer-string) "\n" t) 'rtags-async-rc-sort-lines-compare)))
    (erase-buffer)
    (while lines
      (insert (car lines) "\n")
